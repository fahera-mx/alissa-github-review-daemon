...
| `ALISSA_REVIEW_OPERATORS` | *(empty — no ack honoured)* | logins allowed to re-open a capped PR with `alissa-review: re-enter +N`, one `\|`-separated string; **pass-through**; needs `REVLOOP_VERSION` ≥ 0.16.0 (an older daemon rejects the key it does not know) |
